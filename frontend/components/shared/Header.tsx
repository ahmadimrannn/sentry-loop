...
const Header = () => {
  ...
  return (
    <div className="flex justify-center w-full py-6 px-4">
      <div className="flex items-center justify-between px-6 py-3 bg-transparent backdrop-blur-lg rounded-full shadow-md border w-full max-w-3xl relative z-10">
        <div className="flex items-center">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            {navLinks.map((item) => (
              ...
              >
                <a href={item.path} className="text-sm text-gray-900 hover:text-gray-600 transition-colors font-medium">
                  {item.name}
                </a>
              </motion.div>
            ))}
            ...
            <div className="flex flex-col space-y-6">
              {navLinks.map((item, i) => (
                <motion.div
                  ...
                  exit={{ opacity: 0, x: 20 }}
                >
                  <a href={item.path} className="text-base text-gray-900 font-medium" onClick={toggleMenu}>
                    {item.name}
                  </a>
                </motion.div>
              ))}

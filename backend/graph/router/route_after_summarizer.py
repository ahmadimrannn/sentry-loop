...
def route_after_summarize(state: InvestigationState) -> Command:
    step_count = state.get('step_count', 0)
    if step_count >= MAX_STEPS:
        return Command(update={"status_after_routing": "step_limit_exceeded"}, goto="propose_fix")
    ...
    try:
        ...
        if evidence_check and evidence_check.confident_enough:
            return Command(update={"status_after_routing": "confident_enough_evidence_gathered"}, goto="propose_fix")
    except Exception:
        evidence_check = None
    ...
    try:
        ...
        if not check.learned_something_new:
            return Command(update={"status_after_routing": "didnt_learn_something_new"}, goto="propose_fix")
    except Exception:
        return Command(update={"status_after_routing": "didnt_learn_something_new"}, goto="propose_fix")
